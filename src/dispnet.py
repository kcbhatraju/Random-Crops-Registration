...
from torch import nn, set_grad_enabled, cat, load, tensor
...
class Crop(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if firstcrop[0] > 10: firstcrop[0] = int((firstcrop[0])/25*10) # edge case handling [if firstcrop x is > 110 then secondcrop x can go up to > 110+15=125 (off of picture)]
        if firstcrop[1] > 10: firstcrop[1] = int((firstcrop[1])/25*10) # edge case handling [if firstcrop x is > 110 then secondcrop x can go up to > 110+15=125 (off of picture)]
        firstcrop = tuple(firstcrop)
        
        secondcrop = list(firstcrop)
        xshift = random.randint(0,15)
        yshift = random.randint(0,15)
        secondcrop[0] = firstcrop[0] + yshift
        secondcrop[1] = firstcrop[1] + xshift
        ...
        return (cat([im1,im2]), tensor([xshift, yshift])) # (imgs, labels)
...
def evaluate(type, times=None):
    match type.lower():
        case "train": set, name = trainset, "R"
        case "test": set, name = testset, "T"
        case _: raise ValueError("Invalid evaluation type.")
    
    if times is None: times = len(set)
    
    xflow = DispNet()
    xflow.load_state_dict(load("models/xflow.txt"))
    
    yflow = DispNet()
    yflow.load_state_dict(load("models/yflow.txt"))
    
    summed = 0
    for i in range(min(times,len(set))):
        xflow.eval(), yflow.eval()
        img, lab = set[i]
        x_lab, y_lab = lab
        
        x_probs = xflow.forward(img[None,:])
        y_probs = yflow.forward(img[None,:])
        
        _, x_pred = x_probs.topk(1, dim=1)
        _, y_pred = y_probs.topk(1, dim=1)
        ...
        comb = Image.new("RGB", (pil1.width+x_pred.item(), pil1.height+y_pred.item()))
        comb.paste(pil1, (0,0))
        comb.paste(pil2, (x_pred.item(),y_pred.item()))
        ...
        match orig:
            case "(R)": state = (abs(x_pred.item()-x_lab.item()) <= 3 and abs(y_pred.item()-y_lab.item()) <= 3)
            case "(U)": state = (abs(x_pred.item()-x_lab.item()) > 1 or abs(y_pred.item()-y_lab.item()) > 1)
            case _: raise ValueError("Invalid registration type.")
        
        summed += state
        results = glob.glob("results/dispnet/*/")
        size = len(glob.glob(f"{results[2]}/*/"))
        file = 2-(state)*((orig == "(R)")+1)
        size = len(glob.glob(f"{results[file]}/*/"))
        path = f"{results[file]}{orig[1]}{size+1}/"
        
        with open("logs/dispnet/evaluate_log.txt", "a") as log:
            log.write(f"{name}{orig[1]}{size+1}. True: ({x_lab.item()}, {y_lab.item()}), Predicted: ({x_pred.item()}, {y_pred.item()}) {('(Failed)','(Passed)')[state]}\n")
