...
class Crop(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (cat([im1,im2]), tensor(yshift)) # (imgs, labels)
...
def train(epochs=75):
    model = DispNet()
    try: model.load_state_dict(load("models/yflow.txt"))
    ...
    for e in range(epochs):
        with open("logs/yflow/train_log.txt", "a") as log:
    ...
    save(model.state_dict(),"models/yflow.txt")
...
def evaluate(type, times=25):
    ...
    model = DispNet()
    model.load_state_dict(load("models/yflow.txt"))
    summed = 0
    for i in range(min(times,len(set))):
        ...
        comb = Image.new("RGB", (pil1.width, pil1.height+top_class.item()))
        comb.paste(pil1, (0,0))
        comb.paste(pil2, (0,top_class.item()))
        ...
        summed += state
        results = glob.glob("results/yflow/*/")
        ...
        with open("logs/yflow/evaluate_log.txt", "a") as log:
