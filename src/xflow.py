...
class Crop(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (cat([im1,im2]), tensor(xshift)) # (imgs, labels)
...
def train(epochs=75):
    model = DispNet()
    try: model.load_state_dict(load("models/xflow.txt"))
    ...
    for e in range(epochs):
        with open("logs/xflow/train_log.txt", "a") as log:
    ...
    save(model.state_dict(),"models/xflow.txt")
...
def evaluate(type, times=25):
    ...
    model = DispNet()
    model.load_state_dict(load("models/xflow.txt"))
    summed = 0
    for i in range(min(times,len(set))):
        ...
        comb = Image.new("RGB", (pil1.width+top_class.item(), pil1.height))
        comb.paste(pil1, (0,0))
        comb.paste(pil2, (top_class.item(),0))
        ...
        summed += state
        results = glob.glob("results/xflow/*/")
        ...
        with open("logs/xflow/evaluate_log.txt", "a") as log:
